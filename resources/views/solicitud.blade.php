                ...
                <div class="nalika-profile">
                    <div class="profile-dtl">
                        ...
                        <h2>
                            ...
                            <p style="color: #66FCF1; font-size: 13px;">{{$tipouser->nombre_usuario}}</p>
                        </h2>
                    </div>
                            ...
                            <div class="calender-inner">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <center>
                                            <h4 class="">Estado de solicitudes</h4>
                                        </center>
                                        <hr>
                                    </div>
                                    <center>
                                                ...
                                                <tbody>
                                                    @foreach($mis_solicitudes as $solicitud)
                                                        <tr>
                                                            ...
                                                            @foreach($tipo_solicitud as $tipo)
                                                                @if($tipo->id_tipo_solicitud == $solicitud->id_tipo_solicitud)
                                                                    <td class="tipo">{{$tipo->nombre_tipo}}</td>
                                                                @else
                                                                @endif
                                                            @endforeach
                                                            @if($solicitud->status_detalle == "En Espera")
                                                                <td style="color:red";><b>{{$solicitud->status_detalle}}</b></td>
                                                            @else
                                                                @if($solicitud->status_detalle == "Autorizado")
                                                                    <td style="color:#FFC300";><b>{{$solicitud->status_detalle}}</b></td>
                                                                @else
                                                                    <td style="color:green";><b>{{$solicitud->status_detalle}}</b></td>
                                                                @endif
                                                            @endif
                                                            <td>
                                                            @if($solicitud->status_detalle == "Entregado")
                                                                <a type="button" class="btn btn-danger" href="{{ route('pdf', $solicitud->id_solicitud) }}" target="_blank"><i class="fa fa-file-text"></i> PDF</a>   
                                                            @else
                                                                @if($solicitud->status_detalle == "En Espera")
                                                                    <form method="POST" action="{{ route('autorizarsolicitud') }}">
                                                                        @csrf
                                                                        <input type="hidden" name="id_solicitud" value={{$solicitud->id_solicitud}}>
                                                                        <input type="hidden" name="autorizado" value="Entregado">
                                                                        <a type="button" class="btn btn-danger" href="{{ route('pdf', $solicitud->id_solicitud) }}" target="_blank"><i class="fa fa-file-text"></i> PDF</a>
                                                                        <button type="submit" class="btn btn-success" href="{{ route('autorizarsolicitud') }}"><i class="fa fa-file-text"></i> Autorizar</button>
                                                                    </form>
                                                                @endif
                                                            @endif
                                                            </td>
                                                    ...
                                                    @endforeach
                                                    <div style="color: #C5C6C7;">{{ $mis_solicitudes->appends(request()->query())->links() }}</div>
                                                </tbody>
                                            </table>
                                        </div>
                                    </center>
